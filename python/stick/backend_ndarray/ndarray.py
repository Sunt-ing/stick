...
class NDArray:
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_shape = tuple([old_shape + pads[0] + pads[1] for old_shape, pads in zip(self.shape, axes)])
        new_array = full(new_shape, 0, dtype=self.dtype, device=self.device)
        ranges = [slice(pads[0], old_shape + pads[0]) for old_shape, pads in zip(self.shape, axes)]
        new_array[tuple(ranges)] = self
        return new_array
        ### END YOUR SOLUTION
